...
HUDDLE_UP
SOVEREIGN_BLADE"""
images_to_upload = set(images_to_upload.splitlines())
card_images = os.path.join(base_path, 'card-images')
for filename in os.listdir(card_images):
    if filename.endswith('.png') and filename.replace('Plus1', '').replace('.png', '') in images_to_upload:
